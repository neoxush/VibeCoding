...
class BuildingBlockGenerator:
    ...
    def _create_ramp(self, position: mathutils.Vector, dimensions: mathutils.Vector) -> bpy.types.Object:
        """Create a ramp block (wedge shape sloping from high to low along X)."""
        bpy.ops.mesh.primitive_cube_add(
            size=1.0,
            location=position
        )
        ...
        bmesh.ops.translate(bm, verts=bm.verts, vec=(0, 0, dimensions.z * 0.25))
    ...
    def _process_layer(self, layer: LayerConfig, space, layer_idx: int) -> List[bpy.types.Object]:
        ...
        if layer.rule == PlacementRule.EDGE_LOOP:
            ...
            num_points = max(int(depth * layer.density * 0.1), 1)
            forward = space.orientation @ mathutils.Vector((0, 1, 0))
            half_depth = depth / 2
            for i in range(num_points):
                t = (i / (num_points - 1)) if num_points > 1 else 0.5
                offset = forward * (t * depth - half_depth)
                pos = mathutils.Vector((offset.x, offset.y, 0))
                points.append(pos)
                ...
                if coll:
                    ...
                    if meshes:
                        ...
                        print(
                            ...
                            "has no mesh objects, using fallback cubes"
                        )
                else:
                    print(
                        f"PCG Warning: Collection '{layer.collection_name}' "
                        "not found, using fallback cubes"
                    )
