...
(function () {
      ...
      GM_getTab((tab) => {
        ...
        try {
          const parsed = JSON.parse(
            sessionStorage.getItem("stm_state") || "{}",
          );
          if (parsed.stmRole) {
            resolve({
              role: parsed.stmRole,
  ...
  function createConfigPanel() {
    ...
    panel
      .querySelector("#stm-config-cancel")
      .addEventListener("click", hideConfigPanel);
    panel
      .querySelector("#stm-config-save")
      .addEventListener("click", saveConfigFromPanel);
    panel
      .querySelector("#stm-config-reset")
      .addEventListener("click", resetConfigToDefault);
  ...
  function showConfigPanel() {
    ...
    document.getElementById("stm-source-button").value =
      config.sourceKey.button;
    ...
    document.getElementById("stm-source-shift").checked =
      config.sourceKey.shift;
    document.getElementById("stm-target-button").value =
      config.targetKey.button;
    ...
    document.getElementById("stm-target-shift").checked =
      config.targetKey.shift;
  ...
  function saveConfigFromPanel() {
    const newConfig = {
      sourceKey: {
        button: parseInt(document.getElementById("stm-source-button").value),
        ctrl: document.getElementById("stm-source-ctrl").checked,
        alt: document.getElementById("stm-source-alt").checked,
        shift: document.getElementById("stm-source-shift").checked,
      },
      targetKey: {
        button: parseInt(document.getElementById("stm-target-button").value),
        ctrl: document.getElementById("stm-target-ctrl").checked,
        alt: document.getElementById("stm-target-alt").checked,
        shift: document.getElementById("stm-target-shift").checked,
      },
    };
  ...
  function resetAllRoles() {
    ...
    keys.forEach((k) => {
      ...
      else if (k.startsWith(disconnectPrefix))
        ids.add(k.slice(disconnectPrefix.length));
      else if (k.startsWith(sourcesPrefix))
        ids.add(k.slice(sourcesPrefix.length));
    });
  ...
  function updateUI() {
    ...
    if (!document.body) {
      window.addEventListener("DOMContentLoaded", updateUI, { once: true });
      return;
    }
    ...
    if (!ui) {
      ...
      ui.grip.id = "stm-grip";
      ui.grip.innerHTML =
        '<div class="stm-grip-dot"></div><div class="stm-grip-dot"></div><div class="stm-grip-dot"></div>';
      ...
      window.addEventListener(
        ...
        (e) => {
          if (
            ...
            !ui.container.contains(e.target)
          ) {
            toggleMenu();
            ui.container.classList.add("stm-collapsed");
          }
    ...
    ui.container.style.display =
      myRole === "idle" || isFullscreen ? "none" : "flex";
    ...
    if (
      myRole !== "idle" ||
      (myRole === "idle" && GM_getValue(KEY_LATEST_SOURCE, null))
    ) {
      ui.menu.innerHTML = contextMenuItems
        .map(
          (item) =>
  ...
  function handleGripMouseMove(e) {
    ...
    newTop = Math.max(
      10,
      Math.min(newTop, windowHeight - containerHeight - 10),
    );
  ...
  function setRole(role, id = null, joinExisting = false) {
    if (role === "source") {
      ...
      GM_setValue(KEY_LATEST_SOURCE, {
        sourceId: groupId,
        timestamp: Date.now(),
      });
    } else if (role === "target") {
      if (!id) {
        Notify.error("Cannot become Target without a Source ID.");
        return;
      }
      saveState("target", id);
    }
  ...
  function handleGlobalDragOver(e) {
    if (
      e.dataTransfer.types.includes("application/stm-role-request") ||
      (e.dataTransfer.types.includes("text/plain") &&
        e.dataTransfer.getData("text/plain").startsWith("STM_ROLE:"))
    ) {
      e.preventDefault();
      e.dataTransfer.dropEffect = "copy";
  ...
  function attachRoleSpecificListeners() {
    ...
    const disconnectListener = GM_addValueChangeListener(
      getDisconnectKey(myId),
      (k, o, n, r) => {
    ...
    const globalResetListener = GM_addValueChangeListener(
      KEY_GLOBAL_RESET,
      (k, o, n, r) => {
        if (r) saveState("idle", null, 0, null);
      },
    ...
    if (myRole === "target") {
      ...
      const urlListener = GM_addValueChangeListener(
        getTimestampKey(myId),
        (k, o, n) => {
          if (n > myLastTs) {
      ...
      const retargetListener = GM_addValueChangeListener(
        `${GM_PREFIX}retarget_${myId}`,
        (k, o, n) => {
          if (n && n.newSourceId) {
  ...
  function initialize() {
    ...
    loadState().then((s) => {
      ...
      const staleInterests = GM_listValues().filter((k) =>
        k.startsWith(`${GM_PREFIX}interest_`),
      );
